...
records into a plain-text Markdown report: provenance, budget-performance tables
by strategy, a few computed observations, and explicit limitations. Tables are
...
specific to these synthetic benchmarks, seeds, and budgets, and are not general
claims about context engineering.
"""
...
def _observations(result: ExperimentResult) -> list[str]:
    oracle = next(
        (r for r in result.results if r.strategy_id == _ORACLE), None
    )
...
def render_experiment(result: ExperimentResult) -> str:
    """Render a single experiment result to a Markdown section.
...
def render_report(results: Mapping[str, ExperimentResult]) -> str:
    """Render a full Phase 2 report from named experiment results.
    ...
    """
    sections = [
        "# Phase 2 report: selection under budget pressure",
        "",
        "Controlled, synthetic experiments on the `selection-signal-retrieval`",
        "benchmark family. Results are early and specific to these benchmarks,",
        "seeds, and budgets. `oracle` is an upper bound that reads ground-truth",
        "relevance and is not a deployable strategy.",
        "",
    ]
