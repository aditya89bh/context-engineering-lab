"""Phase 4 Markdown reporting.

Turns one or more :class:`~context_engineering_lab.core.results.ExperimentResult`
records from the temporal experiments into a plain-text Markdown report:
provenance, an ``answer_support`` table and a ``relevant_age_gap`` table by
strategy and budget, a by-metric summary, mechanical observations, and explicit
...
claims about temporal reasoning, and ``oracle-temporal`` is an upper bound that
reads ground-truth relevance and is not deployable.
"""
...
_ORACLE = "oracle-temporal"
_HEADLINE_METRIC = "answer_support"
_AGE_GAP = "relevant_age_gap"
_STALE = "stale_selection_rate"
...
def _observations(result: ExperimentResult) -> list[str]:
    ...
    if oracle is not None:
        ceiling = _mean_overall(oracle, _HEADLINE_METRIC)
        lines.append(
            f"- oracle mean {_HEADLINE_METRIC} = {_fmt(ceiling)} (ceiling, not "
            "deployable)."
        )
    scored: list[tuple[str, float]] = []
    for run in others:
        value = _mean_overall(run, _HEADLINE_METRIC)
    ...
    if scored:
        ...
        lines.append(
            f"- best non-oracle strategy by mean {_HEADLINE_METRIC}: "
            f"{best[0]} ({best[1]:.2f})."
        )
        lines.append(
            f"- weakest non-oracle strategy by mean {_HEADLINE_METRIC}: "
            f"{worst[0]} ({worst[1]:.2f})."
        )
    stale: list[tuple[str, float]] = []
    for run in others:
        value = _mean_overall(run, _STALE)
        if value is not None:
            stale.append((run.strategy_id, value))
    if stale:
        worst_stale = max(stale, key=lambda pair: pair[1])
        lines.append(
            f"- highest mean {_STALE} among non-oracle strategies: "
            f"{worst_stale[0]} ({worst_stale[1]:.2f})."
        )
    return lines


def render_experiment(result: ExperimentResult) -> str:
    """Render a single temporal experiment result to a Markdown section.
    ...
    lines = [
        ...
        f"- budgets: {budgets}",
        "",
        f"### {_HEADLINE_METRIC} by strategy and budget",
        "",
        *_metric_table(result, _HEADLINE_METRIC),
        "",
        f"### {_AGE_GAP} by strategy and budget (lower is better)",
        "",
        *_metric_table(result, _AGE_GAP),
        "",
...
def render_report(results: Mapping[str, ExperimentResult]) -> str:
    """Render a full Phase 4 report from named experiment results.
    ...
    """
    sections = [
        "# Phase 4 report: temporal context under budget pressure",
        "",
        "Controlled, synthetic experiments on the `temporal-context-relevance`",
        "benchmark family. Phase 4 studies temporal *effects* only; it implements",
        "no forgetting or retention policy. Results are early and specific to",
        "these benchmarks, seeds, and budgets. `oracle-temporal` is an upper bound",
        "that reads ground-truth relevance and is not a deployable strategy.",
        "",
    ]
    ...
    sections.extend(
        [
            ...
            "- Time is modeled as integer positions, not wall-clock timestamps.",
            "- No forgetting or retention is studied; only temporal selection.",
            "- Metrics are aggregated over a small number of seeds.",
            "- `oracle-temporal` results are a ceiling, not an achievable target.",
            "- Observations describe these benchmarks only; they are not general",
            "  claims about temporal reasoning.",
            "",
        ]
    )
    return "\n".join(sections)
